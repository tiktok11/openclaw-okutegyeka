...
// ---------------------------------------------------------------------------
// Windows stub (openssh crate doesn't compile on Windows)
// ---------------------------------------------------------------------------
...
mod inner {
    ...
    const NOT_SUPPORTED: &str = "SSH remote connections are not supported on Windows";

    pub struct SshConnectionPool;

    impl SshConnectionPool {
        pub fn new() -> Self { Self }
        pub async fn connect(&self, _config: &SshHostConfig) -> Result<(), String> { Err(NOT_SUPPORTED.into()) }
        pub async fn disconnect(&self, _id: &str) -> Result<(), String> { Err(NOT_SUPPORTED.into()) }
        pub async fn is_connected(&self, _id: &str) -> bool { false }
        pub async fn get_home_dir(&self, _id: &str) -> Result<String, String> { Err(NOT_SUPPORTED.into()) }
        pub async fn resolve_path(&self, _id: &str, _path: &str) -> Result<String, String> { Err(NOT_SUPPORTED.into()) }
        pub async fn exec(&self, _id: &str, _command: &str) -> Result<SshExecResult, String> { Err(NOT_SUPPORTED.into()) }
        pub async fn exec_login(&self, _id: &str, _command: &str) -> Result<SshExecResult, String> { Err(NOT_SUPPORTED.into()) }
        pub async fn sftp_read(&self, _id: &str, _path: &str) -> Result<String, String> { Err(NOT_SUPPORTED.into()) }
        pub async fn sftp_write(&self, _id: &str, _path: &str, _content: &str) -> Result<(), String> { Err(NOT_SUPPORTED.into()) }
        pub async fn sftp_list(&self, _id: &str, _path: &str) -> Result<Vec<SftpEntry>, String> { Err(NOT_SUPPORTED.into()) }
        pub async fn sftp_remove(&self, _id: &str, _path: &str) -> Result<(), String> { Err(NOT_SUPPORTED.into()) }
    }
